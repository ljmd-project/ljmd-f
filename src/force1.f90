...
SUBROUTINE MORSE
 IMPLICIT NONE
  ...
  REAL(kind=dbl) :: rsq, rcutsq, rinv, pos1(3), delta(3)
  REAL(kind=dbl) :: boxby2, c12, c6, r6, ffac,re,morsee,a1,epot1,ffac1
  INTEGER :: i, j, k, n, m, ii, jj, kk, tid, fromidx, toidx
  INTEGER, EXTERNAL :: omp_get_thread_num
     ...
     DO j=1,npercell(i)-1
        ...
        DO k=j+1,npercell(i)
           ...
           IF (rsq < rcutsq) THEN
             ...
             ! r6 = rinv*rinv*rinv
            !  ffac = (12.0_dbl*c12*r6 - 6.0_dbl*c6)*r6*rinv
            ffac1=2*De*a1*(1-exp(-a1*(r-re)))*(exp(-a1*(r-re))
            epot1 = epot1 + morsee
             ! epot = epot + r6*(c12*r6 - c6)

              frc(ii,:,tid) = frc(ii,:,tid) + delta*ffac1
              frc(jj,:,tid) = frc(jj,:,tid) - delta*ffac1
           END IF
        END DO
     END DO
  END DO
